...
const inputVariants = cva(
  ...
  'w-full min-w-0 bg-card transition-[color,box-shadow] outline-none file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive',
  {
    variants: {
      variant: {
        default:
          'h-11 rounded-md border border-input px-2.5 py-1 text-base focus-visible:border-foreground focus-visible:ring-1 focus-visible:ring-foreground focus-visible:ring-inset aria-invalid:ring-3 aria-invalid:ring-destructive/20 md:text-sm',
        // The app's standard form field: hairline lift and a border-only focus
        // treatment instead of a ring.
        field:
          'h-11 rounded-md border border-border px-3.5 text-base shadow-hairline focus-visible:border-foreground focus-visible:ring-1 focus-visible:ring-foreground focus-visible:ring-inset md:text-sm',
        // A compact cell inside a denser row — a parsed meal item.
        cell: 'h-9 rounded-sm border border-border px-2 text-center text-base tabular-nums focus-visible:border-foreground focus-visible:ring-1 focus-visible:ring-foreground focus-visible:ring-inset md:text-caption',
