...
//
// No surface of its own: the caller supplies the card, so the rows can run
// edge to edge under a header that matches the cards beside it.
    ...
    <Accordion multiple>
      {groups.map((group) => (
        <AccordionItem key={group.mealType} value={group.mealType}>
          <AccordionTrigger className="px-5 hover:no-underline hover:bg-accent/15 rounded-s-none rounded-e-none items-center">
